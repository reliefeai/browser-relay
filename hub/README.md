...
- `GET  /v1/device/connect?routeId=...` — the browser extension's outbound WebSocket. The device sends its secret in the first `device.auth` frame, never in the URL.
...
For rollout compatibility, the Hub temporarily accepts older extensions that send
`?token=...`. New extension code intentionally has no query-token fallback. Deploy the
new Hub before updating extensions, then remove legacy acceptance after old clients have
aged out.
